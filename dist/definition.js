...
function collectNewTypeDefinitions(allDefinitions, definitionPool, newDefinition, schemaMap) {
    ...
    if (newDefinition.kind === 'InterfaceTypeDefinition') {
        ...
        newDefinition.fields.forEach(collectNode);
        var interfaceImplementations = allDefinitions.filter(function (d) { return d.kind === 'ObjectTypeDefinition' && d.interfaces.some(function (i) { return i.name.value === interfaceName_1; }); });
    ...
    function collectNode(node) {
        ...
        if (['AWSURL', 'AWSDateTime', 'AWSIPAddress', 'AWSDate', 'AWSTime', 'AWSTimestamp', 'AWSEmail', 'AWSJSON', 'AWSURL', 'AWSPhone'].includes(nodeTypeName))
            return;
        // collect missing argument input types
        if (!definitionPool.some(function (d) { return d.name.value === nodeTypeName; }) && !lodash_1.includes(builtinTypes, nodeTypeName)) {
    ...
    function collectDirective(directive) {
        ...
        if (['aws_auth', 'model', 'searchable'].includes(directiveName))
            return;
        if (!definitionPool.some(function (d) { return d.name.value === directiveName; }) && !lodash_1.includes(builtinDirectives, directiveName)) {
